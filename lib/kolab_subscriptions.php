...
class kolab_subscriptions
{
    ...
    public function list_folders($type)
    {
        ...
        if ($this->dav) {
            if ($type == 'note') {
                ...
                if ($storage->get_error_code()) {
                    rcube::raise_error([
                        'code' => 500, 'type' => 'php', 'line' => __LINE__, 'file' => __FILE__,
                        'message' => "IMAP error: " . $storage->get_error_str(),
                    ], true, true);
                }
                ...
                // FIXME error handling. There is no way to detect an error at the moment.
                $result = array_map([$this, 'dav_folder_prop'], $result);
            }
        } else {
            $folderdata = [];
            $result = kolab_storage::list_folders('', '*', $type, false, $folderdata, true);
            $result = array_map([$this, 'imap_folder_prop'], $result);
        }
     ...
     *
     * @return int|false false if not set, otherwise the flag.
     */
